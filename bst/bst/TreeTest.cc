...
struct ScoringListener : Catch::EventListenerBase
{
  ...
  {
	  if (sectionStats.sectionInfo.name.back() != ']')
		{
			return;
		}
	  const auto begin_idx = sectionStats.sectionInfo.name.find_last_of('[') + 1;
	  const auto end_idx = sectionStats.sectionInfo.name.size() - 1;
	  const double score = stod(sectionStats.sectionInfo.name.substr(begin_idx, end_idx - begin_idx));
	  
	  // Point value is multiplied by total total points for the tests as a percentage.
	  double point_value_num = score / 100.0 * POINTS;
	  if (sectionStats.assertions.total () > 0)
		{
			if (!sectionStats.assertions.allPassed ())
			{
				...
				stream << "- " <<  fixed << setprecision (3) << point_value_num << " points:   " << sectionStats.sectionInfo.name;
...
main (int argc, char* argv[])
{
  ...
  std::cout << "\n\n{ \"scores\": {\"auto\": " << FINAL_SCORE << " } }" << std::endl;
  return result;
}
